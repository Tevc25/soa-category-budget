...
async def create_category(user_id: str = Path(...), payload: CategoryRequest = Body(...)):
    try:
...
async def get_categories(user_id: str = Path(...)):
    return category_service.get_categories(user_id)

@router.put("/categories/{category_id}/update", status_code=status.HTTP_200_OK)
async def update_category(user_id: str = Path(...), category_id: str = Path(...), payload: CategoryRequest = Body(...)):
    try:
...
async def delete_category(user_id: str = Path(...), category_id: str = Path(...)):
    try:
...
async def upsert_budget(user_id: str = Path(...), payload: BudgetRequest = Body(...)):
    try:
...
async def get_budgets(user_id: str = Path(...), month: str | None = Query(None)):
    try:
...
async def delete_budget(user_id: str = Path(...), budget_id: str = Path(...)):
    try:
